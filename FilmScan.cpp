...
bool Video::ReadNextFrame(size_t currfnum)
{
    ...
    // Clear the frame before decoding
    av_frame_unref(this->frameNative);

    while (av_read_frame(this->format, &packet) >= 0) {
        if (packet.stream_index == this->streamIdx) {
            // Send the packet to the decoder
            ret = avcodec_send_packet(this->codec, &packet);
            ...
            while (true) {
                ret = avcodec_receive_frame(this->codec, this->frameNative);
...
bool FilmScan::SourceLibAV(const std::string filename)
{
	...
	if((this->numFrames=vid->format->streams[vid->streamIdx]->nb_frames) == 0)
	{
		...
		this->numFrames =
			(int)(
				(vid->format->streams[vid->streamIdx]->duration *
				av_q2d(vid->format->streams[vid->streamIdx]->time_base)
				) + 0.5
			);
	...
	// Modern FFmpeg
	AVCodecParameters *codecpar = vid->format->streams[vid->streamIdx]->codecpar;
	...
	if (!vid->codec) {
		...
		delete vid;
		vid = NULL;
		throw AeoException("Couldn't allocate codec context");
	}
	if (avcodec_parameters_to_context(vid->codec, codecpar) < 0) {
		avformat_close_input(&(vid->format));
		delete vid;
		vid = NULL;
		throw AeoException("Couldn't copy codec parameters");
	}
